...
namespace textlayout // from https://github.com/ChenlizheMe/Infernux
{
    ...
    namespace detail
    {
        ...
        {
#ifndef NDEBUG // 在主线程使用
            static const std::thread::id s_ownerThread = std::this_thread::get_id();
            assert(std::this_thread::get_id() == s_ownerThread && "TextLayout font cache accessed from non-owner thread");
#endif
        }
    } // namespace detail
    ...
    inline void PushLine(TextLayoutResult& result, const char* textBegin, const char* start, const char* end, float width)
    {
        ...
        result.totalWidth = max(result.totalWidth, width);
    }
    ...
    inline TextLayoutResult LayoutText(const TextLayoutParams& params)
    {
        ...
        result.baseLineHeight =
            result.font != nullptr
            ? max(result.font->CalcTextSizeA(result.fontSize, FLT_MAX, 0.0f, "A").y, result.fontSize)
            : result.fontSize;
        result.lineAdvance = result.baseLineHeight * max(params.lineHeight, 0.1f);
