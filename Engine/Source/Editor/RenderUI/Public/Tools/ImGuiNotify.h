...
#define NOTIFY_FADE_IN_OUT_TIME				75			// Fade in and out duration
...
#define NOTIFY_RENDER_OUTSIDE_MAIN_WINDOW	true		// If true, the notifications will be rendered in the corner of the monitor, otherwise in the corner of the main window
    ...
    */
    ...
    class RENDERUI_API ImGuiToastManager : public Singleton<ImGuiToastManager>
    {
    public:
        ...
        // Replace the entire notification list
        void SetNotifications(const std::vector<ImGuiToast>& notifications);

        // Add a single notification to the list
        void AddNotification(const ImGuiToast& toast);

        // Remove a notification at the specified index
        void RemoveNotification(size_t index);

        // Get the notification list (modifiable)
        std::vector<ImGuiToast>& GetNotifications();

        // Get the notification list (read-only)
        const std::vector<ImGuiToast>& GetNotifications() const;
    ...
    namespace Notify
    {
         ...
         */
        RENDERUI_API void InsertNotification(const ImGuiToast& toast);
         ...
         */
        RENDERUI_API void RemoveNotification(size_t index);
         ...
         */
        RENDERUI_API void RenderNotifications();
    }
}
